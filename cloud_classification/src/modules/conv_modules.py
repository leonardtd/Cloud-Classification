...
class CloudNet(nn.Module):
    ...
    def forward(self, x):
        x = nn.GELU()(self.b1(self.conv1(x)))
        x = self.pool(x)
        x = nn.GELU()(self.b2(self.conv2(x)))
        x = self.pool(x)
        x = nn.GELU()(self.b3(self.conv3(x)))
        x = nn.GELU()(self.b4(self.conv4(x)))
        x = self.pool(x)
        x = x.flatten(1)
        x = F.dropout(nn.GELU()(self.fc1(x)), self.dropout, training=self.training)
        x = F.dropout(nn.GELU()(self.fc2(x)), self.dropout, training=self.training)
        x = self.fc3(x)
        return x
